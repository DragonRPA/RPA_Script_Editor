...
### 🔍 [HTML 보기 버튼 및 계층 경로 노출 탑재] 웹 DOM 엔진 최우선 가동 및 상세 검사기

1. **[HTML 보기] 전 요소 전용 검사 버튼 추가**:
   - 각 조작 객체 카드 우측에 `[HTML 보기]` 버튼을 추가하여, 클릭 시 해당 요소의 실제 HTML 코드(`outerHTML`)와 세부 속성을 팝업 모달로 즉시 확인 및 원클릭 복사 가능
2. **3~4단계 상위 조상 계층 경로(`경로: A > B > C`) 카드 전면 노출**:
   - 요소의 라벨뿐만 아니라 상위 컨테이너와 섹션의 텍스트 체인을 `경로: 계약관리 > 계약조회 > [계약번호]` 형태로 노출하여 사용자가 화면 내 위치를 100% 명확히 인지
3. **웹 대상 브라우저 실제 DOM 엔진 최우선 가동**:
   - 대상 URL 또는 브라우저 선택 시 OS UIA 단순 컨트롤 순회 대신, 진정한 웹 DOM 파서를 가동하여 `<select>` 드롭다운 및 `.input-group`을 무누락 전수 수집
